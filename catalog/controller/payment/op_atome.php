...
class ControllerPaymentOPAtome extends Controller {
	...
	public function confirm()
	{
		...
		$this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('op_atome_default_order_status_id'));
	}
	...
	protected function index() {
		...
		$this->id = 'payment';
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/op_atome.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/op_atome.tpl';
		} else {
			$this->template = 'default/template/payment/op_atome.tpl';
		}	
		
		$this->render();
	}
	
	public function op_atome_form()
	{
		
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/op_atome_iframe.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/op_atome_iframe.tpl';
		} else {
			$this->template = 'default/template/payment/op_atome_iframe.tpl';
		}
	...
	public function op_atome() {
		...
		if (!empty($order_info)) {
			...
			$this->load->model('payment/op_atome');
			$product_info = $this->model_payment_op_atome->getOrderProducts($this->session->data['order_id']);
			
			//获取订单详情
			$productDetails = $this->getProductItems($product_info);
			//获取消费者详情
			$customer_info = $this->model_payment_op_atome->getCustomerDetails($order_info['customer_id']);
			...
			//提交网关
			$action = $this->config->get('op_atome_transaction');
			$this->data['action'] = $action;
			
			//账户号
			$account = $this->config->get('op_atome_account');
			...
			//终端号
			$terminal = $this->config->get('op_atome_terminal');
			$this->data['terminal'] = $terminal;
			
			//securecode
			$securecode = $this->config->get('op_atome_securecode');
			
			//返回地址
			$backUrl = $base_url.'index.php?route=payment/op_atome/callback';
			$this->data['backUrl'] = $backUrl;
			
			//服务器响应地址
			$noticeUrl = $base_url.'index.php?route=payment/op_atome/notice';
			...
			//支付模式Pay Mode
			if($this->config->get('op_atome_pay_mode') == 1){
				//内嵌Iframe
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/op_atome_iframe.tpl')) {
					$this->template = $this->config->get('config_template') . '/template/payment/op_atome_iframe.tpl';
				} else {
				...
				//跳转Redirect
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/op_atome_form.tpl')) {
					$this->template = $this->config->get('config_template') . '/template/payment/op_atome_form.tpl';
				} else {
					$this->template = 'default/template/payment/op_atome_form.tpl';
				}
	...
	public function callback() {
			...
			$this->language->load('payment/op_atome');
			...
			$code_mode = $this->config->get('op_atome_code');
			...
			//匹配终端号
			if($terminal == $this->config->get('op_atome_terminal')){
				//普通终端号
				$securecode = $this->config->get('op_atome_securecode');
			}else{
				$securecode = '';
			...
			if (strtoupper($local_signValue) == strtoupper($back_signValue)) {     //数据签名对比
				...
				if($response_type == 0){		
					...
					if(substr($payment_details,0,5) == 20061){	 
						//排除订单号重复(20061)的交易	
						if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/op_atome_failure.tpl')) {
							$this->template = $this->config->get('config_template') . '/template/payment/op_atome_failure.tpl';
						} else {
							$this->template = 'default/template/payment/op_atome_failure.tpl';
						}
						
					}else{
						if ($payment_status == 1 ){  
							//交易成功
							$this->model_checkout_order->update($this->request->post['order_number'], $this->config->get('op_atome_success_order_status_id'), $message, FALSE);
							
							unset($this->session->data['coupon']);
							
							$this->data['continue'] = HTTPS_SERVER . 'index.php?route=checkout/success';
							if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/op_atome_success.tpl')) {
								$this->template = $this->config->get('config_template') . '/template/payment/op_atome_success.tpl';
							} else {
								$this->template = 'default/template/payment/op_atome_success.tpl';
							}
							...
							$this->model_checkout_order->update($this->request->post['order_number'], $this->config->get('op_atome_failed_order_status_id'),$message, FALSE);
	...
	public function notice() {
		...
		if($this->xml_parser($xml_str)){
			...
			//匹配终端号
			if($_REQUEST['terminal'] == $this->config->get('op_atome_terminal')){
				//普通终端号
				$securecode = $this->config->get('op_atome_securecode');
			}else{
				$securecode = '';
			...
			if (strtoupper($local_signValue) == strtoupper($_REQUEST['signValue'])) {
				...
				if($errorCode == 20061){
					...
					if ($_REQUEST['payment_status'] == 1 ){
						//交易成功
						$this->model_checkout_order->update($_REQUEST['order_number'], $this->config->get('op_atome_success_order_status_id'), $message, false);
					}elseif ($_REQUEST['payment_status'] == -1){
						//交易待处理
						//是否预授权交易
						if($_REQUEST['payment_authType'] == 1){
						...
						$this->model_checkout_order->update($_REQUEST['order_number'], $this->config->get('op_atome_pending_order_status_id'), $message, false);
					}else{
						//交易失败
						$this->model_checkout_order->update($_REQUEST['order_number'], $this->config->get('op_atome_failed_order_status_id'), $message, false);
					}
				}	
			}
			echo "receive-ok";
